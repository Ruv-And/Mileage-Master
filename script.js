      ...
      <div class="wrapper1">
        <nav>
          ...
          <ul>
            ...
            <li class="headerLink shopHTML"><a href="shop.html" onclick="hideMenu()">Buy</a></li>
            <li class="headerLink sellHTML"><a href="sell.html" onclick="hideMenu()">Sell/Trade</a></li>
            <li class="headerLink aboutHTML"><a href="about.html" onclick="hideMenu()">About</a></li> 
            <li class="headerLink contactHTML"><a href="contact.html" onclick="hideMenu()">Contact</a></li>
          </ul>
        </nav>
...
let filterTab = document.getElementById("filter");
let media = window.matchMedia("(max-width: 700px)");
if(filterToggle != null && media){
    filterToggle.addEventListener("click", function(){
        if(!filterOut){ //if the filter tab is currently in (move it out)
            filterToggle.style.left = '70vw !important';
            filterTab.style.left = '0 !important'; //move the filter out
            filterTab.style.boxShadow = '5px 0px 5px rgba(0,0,0,0.7)';
            filterOut = true;
        }
        else{ //if the filter tab is currently out (move it in)
            filterToggle.style.left = '0 !important';
            filterTab.style.left = '-70vw !important'; //move the filter in
            filterTab.style.boxShadow = 'none';
            filterOut = false;
        }
    });
}
...
<tr>
  ...
  <div class="carYear" style="display: none;">${car.year}</div>
  <div class="carMake" style="display: none;">${car.make}</div>
  <div class="carModel" style="display: none;">${car.model}</div>
</div>
`
...
function checkSearchItems(items, carItems){
    for(let i = 0; i < items.length; i++){
        if(items[i].replace(/\s/g, '').length){
            if(!carItems.includes(items[i])) return false;
        }
    }
    return true;
}
        ...
        if(document.querySelector(".searchBar").value == ""){
            ...
            document.querySelectorAll(".car-card-col").forEach(card => {
                card.addEventListener("click", function(){
                    ...
                    selectedCar.carYear = card.querySelector(".carYear").innerHTML;
                    selectedCar.carMake = card.querySelector(".carMake").innerHTML;
                    selectedCar.carModel = card.querySelector(".carModel").innerHTML;
            ...
            allCars.forEach(car => {
                ...
                let isValidCar = checkSearchItems(searchItems, carItems);
                if(isValidCar){
...
window.onclick = function (event) {
    if (!event.target.matches(".select") && !event.target.matches(".filter-line") && !event.target.matches(".selected")) {
        document.querySelectorAll(".menu").forEach(menus => {
            menus.classList.remove("menu-open");
        });
        document.querySelectorAll(".caret").forEach(caret => {
...
window.onload = function(){
    if(document.querySelector(".outerCarDiv") != null){
        ...
        document.querySelector(".carFullName2").innerHTML = selectedCar.carFullName;
        document.querySelector(".carYear").innerHTML = selectedCar.carYear;
        document.querySelector(".carMake").innerHTML = selectedCar.carMake;
        document.querySelector(".carModel").innerHTML = selectedCar.carModel;
    ...
    //APPLY UNDERLINE TO CURRENT PAGE IN HEADER
    document.querySelector(".indexHTML").classList.remove("underlined");
    document.querySelector(".shopHTML").classList.remove("underlined");
    document.querySelector(".sellHTML").classList.remove("underlined");
    document.querySelector(".aboutHTML").classList.remove("underlined");
    document.querySelector(".contactHTML").classList.remove("underlined");
    if(window.location.pathname == "/index.html")
        document.querySelector(".indexHTML").classList.add("underlined");
    if(window.location.pathname == "/shop.html")
        document.querySelector(".shopHTML").classList.add("underlined");
    if(window.location.pathname == "/sell.html")
        document.querySelector(".sellHTML").classList.add("underlined");
    if(window.location.pathname == "/about.html")
        document.querySelector(".aboutHTML").classList.add("underlined");
    if(window.location.pathname == "/contact.html")
        document.querySelector(".contactHTML").classList.add("underlined");

    //apply font
    currFont = sessionStorage.getItem("currFont");
    document.querySelectorAll("*").forEach(function(el) {
        if(notFont(el)){
            el.style.fontFamily = currFont;
        }
    });
    changeCheckmarks();
}
...
let deut = document.querySelector(".deut");
let trit = document.querySelector(".trit");
...
if(deut != null){
    deut.addEventListener("click", function(){
        colorblindSetting = 2;
        sessionStorage.setItem("colorblind", colorblindSetting);
        changeColorblind();
        updateColorblind();
...
if(trit != null){
    trit.addEventListener("click", function(){
        colorblindSetting = 3;
        sessionStorage.setItem("colorblind", colorblindSetting);
        changeColorblind();
        updateColorblind();
...
let regFont = document.querySelector(".regularFont");
let dysFont = document.querySelector(".dyslexiaFont");
let accBtn = document.querySelector(".accessibilityIcon");
let accCon = document.querySelector(".accContainer");
let acc = document.querySelector(".accessibility");
let ogFont = document.body.style.fontFamily;
let currFont = ogFont;
let accOpen = false;
...
if(regFont != null){
    regFont.addEventListener("click", function(){
        //switch check mark
        document.querySelector(".regularFontCheck").classList.remove("hidden");
        document.querySelector(".dyslexiaFontCheck").classList.add("hidden");

        //switch underline
        regFont.style.textDecoration = "underline";
        dysFont.style.textDecoration = "none";

        //switch font
        document.querySelectorAll("*").forEach(function(el) {
            if(notFont(el)){
                el.style.fontFamily = ogFont;
                currFont = ogFont;
                sessionStorage.setItem("currFont", currFont);
            }
        });
    });
    dysFont.addEventListener("click", function(){
        //switch check mark
        document.querySelector(".regularFontCheck").classList.add("hidden");
        document.querySelector(".dyslexiaFontCheck").classList.remove("hidden");

        //switch underline
        regFont.style.textDecoration = "none";
        dysFont.style.textDecoration = "underline";

        //switch font
        document.querySelectorAll("*").forEach(function(el) {
            if(notFont(el)){
                el.style.fontFamily = "Dyslexia";
                currFont = "Dyslexia";
                sessionStorage.setItem("currFont", currFont);
            }
        });
    ...
    accBtn.addEventListener("click", function(){
        if(accOpen){
            accCon.classList.add("hidden");
            accOpen = false;
        }
        else{
            accCon.classList.remove("hidden");
            accOpen = true;
        }
    });
    document.addEventListener("click", function(event){
        if(!acc.contains(event.target)){
            if(accOpen){
                accCon.classList.add("hidden");
                accOpen = false;
            }
...
function notFont(e){
    return e.localName != "img" && e.classList != "material-icons" && e.classList != "cbRow" && e.classList != "socialContainer2" && e.classList != "team" && e.classList != "searchDiv";
}

function changeCheckmarks(){
    if(regFont != null){
        if(sessionStorage.getItem("currFont") == "Dyslexia"){
        //switch check mark
        document.querySelector(".regularFontCheck").classList.add("hidden");
        document.querySelector(".dyslexiaFontCheck").classList.remove("hidden");
...
function changeColorblind(){
    if(none != null){
        let currColor = sessionStorage.getItem("colorblind");
        if(currColor == 0){
            none.src = "images/icons/noneSel.png";
            prot.src = "images/icons/prot.png";
            deut.src = "images/icons/deut.png";
            trit.src = "images/icons/trit.png";
        }
        else if(currColor == 1){
            none.src = "images/icons/none.png";
            prot.src = "images/icons/protSel.png";
            deut.src = "images/icons/deut.png";
            trit.src = "images/icons/trit.png";
        }
        else if(currColor == 2){
            none.src = "images/icons/none.png";
            prot.src = "images/icons/prot.png";
            deut.src = "images/icons/deutSel.png";
            trit.src = "images/icons/trit.png";
        }
        else if(currColor == 3){
            none.src = "images/icons/none.png";
            prot.src = "images/icons/prot.png";
            deut.src = "images/icons/deut.png";
            trit.src = "images/icons/tritSel.png";
        }
    }
}
...
//     if(firstname != "" && lastname != "" && em != "" && msg != ""){
//         var templateParams = {
//             name: 'James',
//             notes: 'Check this out!'
//         };
        
//           const serviceID = "service_jrhqutz";
//           const templateID = "template_cygjrn3";
